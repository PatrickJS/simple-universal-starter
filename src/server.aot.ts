...
// const redis_host = process.env.REDISHOST || '//127.0.0.1:6379';
...
function ngApp(req, res) {
  ...
  if (inMemory[FULL_URL]) {
    return res.send(inMemory[FULL_URL]);
  }
  ...
  Zone.current.fork({ name: 'CSR fallback', onHandleError }).run(() => {
    ...
    }, function transform(err, html) {
      if (err) {
        console.log('Error transforming from render', err);
      }
